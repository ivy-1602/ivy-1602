# hii, I'm Uma Salunke ;)

**AI/ML Engineering Student | Systems & Designs | Creative Writing**..

I build tools that are intentional, calm, and human-centered, where technology meets thoughtful design.

---

### 🍷What I Build

**Android Development**
- [Cursive](https://github.com/ivy-1602/Cursive) — Privacy-first mood journaling app (Kotlin, MVVM, Firebase, Material Design)
- [LetsConnect](https://github.com/ivy-1602/LetsConnect) — Mentor-mentee matching platform

**AI/ML Projects**
- GNSS Satellite Error Prediction Model (with full-stack implementation)
- Various ML pipelines exploring NLP, neural networks, and applied problem-solving

**Web & Systems**
- E-commerce platform for mobile retail
- UI/UX experiments and system design builds

---

### 💻 Tech Stack

**Mobile**: Kotlin • MVVM Architecture • Firebase • Material Design  
**AI/ML**: Python • TensorFlow • scikit-learn • NLP  
**Web**: React • Node.js • Express • Frontend/Backend fundamentals  
**Design**: Figma • UI/UX • Product thinking  

---

### 🦩 Beyond Code

I don't just write code — I write about it too.

- **Assistant Editor** to an anonymous author  
- **Creative Content Writing Lead** @localdev
- Vigorous about making complex ideas feel simple and human  
- Scored 97/100 in English (CBSE Boards, topped Maharashtra) [i 🤍 english lit]

I believe good products need good words. My technical background helps me understand *how* things work; my writing helps me explain *why* they matter.

---

### 🏆 Highlights

🥇 Best Research-Based Report Award (Narcotics Control)  
🎨 Winner — Prompt-Driven UI Design Contest  
♟️ District Rank 1 — Chess  
🏸 District Rank 1 — Badminton  
🏀 Girls Basketball Team Captain  

**Professional Exposure**: EA • Goldman Sachs • AWS (Job Simulations)

---

### 📫Let's Connect

[LinkedIn](https://www.linkedin.com/in/umasalunke7) • [Email](mailto:umasalunke7@gmail.com)

---

*Building at the intersection of code, design, and clarity.*
